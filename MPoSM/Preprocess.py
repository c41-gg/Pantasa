...
def load_dataset(file_path):
    ...
    # Stream the dataset from the 'train' split
    for example in dataset['train']:
        # Each example contains a 'text' field, which is the sentence.
        identifier = str(example['text'][:10])  # Use the first 10 characters as an identifier for demonstration
        sentence = example['text']
        yield identifier, sentence  # Yield both identifier and sentence

def preprocess_text(output_file, batch_size=5000):
    """
    ...
    with open(output_file, 'a', encoding='utf-8') as output:
        ...
        for identifier, sentence in load_dataset():
